...
%\documentclass[dvipdfm]{sig-alternate-2006}

\documentclass{sig-alternate-2006}
...
\title{In-Band Synchronization for\\Distributed SDN Control Planes\thanks{Supported by the European Research Council (ERC) Starting Grant no.~259085, the
...
\numberofauthors{3}
\author{
\alignauthor Liron Schiff \\
\affaddr{Tel Aviv University, Israel} \\
%\affaddr{ACM} \\
%\affaddr{New York, NY} \\
%\email{griscti@hq.acm.org}
\alignauthor Stefan Schmid \\
\affaddr{Aalborg University, Denmark} \\
%\affaddr{ACM} \\
%\affaddr{New York, NY} \\
%\email{murray@hq.acm.org}
\alignauthor Petr Kuznetsov \\
\affaddr{T\'el\'ecom ParisTech, France} \\
%\affaddr{ACM} \\
%\affaddr{New York, NY} \\
...
postulated in STN~\cite{infocom15}. %\petr{We do not seem to discuss the latter?}
...
\paragraph{Motivation and Example}
...
%
We distinguish between two parts of the configuration space of a switch:
...
%\liron{all these jumps from atomic to transactional and back is confusing...}
%

%For this purpose, we introduce two constructs:
...
Before discussing applications in more detail, 
as a case study and in order to introduce some notation, 
we in this section show how to implement
our transactional approach using today's standard SDN protocol,
 OpenFlow  (version 1.4~\cite{openflow}).\footnote{We note
however that hardware OpenFlow switches do not always 
follow the spec as they should~\cite{know}, and will report on a
prototype implementation later in this paper.} 
...
%  configuration?}
%\liron{I am not sure if this is atomic, considering that we execute other things in a bundle then this is practically atomic for us. Perhaps we can leave it unspecified as it is now?}
%[[PK moved this discussion to the algorithms 6 and 7]]
...
%, leaving the remaining fields at default
%values.
%\liron{this is not accurate, the match part simply considers only the metadata field. no need to discuss other fields, in any case matching doesn't change fields and doesn't assume any values to other fields.}
...
the match field, which allows us to ensure that the entry is uniquely
...
the first such command succeeds, 
and subsequent ones will do no harm.
%]]
...
%\liron{SECOND TRY: Note that comparing an address for the first time, without writing it first, always succeeds and also writes to it the compared value. In case where such comparison may occur to some address $addr$, it is advised to first perform a safe initialization by calling $\textit{\compare}(\addr,0)$ which fails in case that $addr$ has already been used (and the initialization is not desired).}
...
discussed in Figure~\ref{fig:CAS-example}.
...
\begin{algorithm}[t]
    ...
    \Ensure installed policy is consistent with previous one
 		%\Do
 		\Repeat
 			\State $\pid,\textit{policy}\gets$ \textbf{read-state} \label{line:readstate}%switch configuration}
...
In practice, we can use a modulo scheme on the 32-bit field of a flow entry
used for storing the policy identifiers, which, in principle,
may lead to inconsistencies if controllers proceed in different speeds.
...
Then the controller chooses any \emph{unused} id and computes the update commands to be executed (Line~\ref{update:compute}). %\liron{I have replaced "claims the first" with "chooses an" following Petr email}
...
%[[PK 
%We observe that our  algorithm is only
...
%wait-free by reading the state of the switch and claiming the current
%policy id within the same {\execatomic} construct.
%]]    

\vspace{1mm}
\noindent\textbf{A note on efficiency.}
%
Our implementations of synchronization primitives
...
The code is made available online\footnote{See \url{https://github.com/lironsc/of-sync-lib}.}. 
%\liron{perhaps we can put it as reference? (putting the link in bib)}
...
(controllers), concurrently issue $1000$ concurrent policy updates
...
%Moreover, we will consider adding special
%locking/synchronization primitives to OpenFlow. 
