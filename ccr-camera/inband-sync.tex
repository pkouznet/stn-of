...
today's OpenFlow. \liron{two times "later"!}
...
\petr{Is it atomic? Should we say that the command returns a ``snapshot'' of the
  configuration?}\liron{I am not sure if this is atomic, considering that we execute other things in a bundle then this is practically atomic for us. Perhaps we can leave it unspecified as it is now?}
...
values.\liron{this is not accurate, the match part simply considers only the metadata field. no need to discuss other fields, in any case matching doesn't change fields and doesn't assume any values to other fields.}
...
for a different policy (Lines~\ref{update:read-state1}-\ref{update:read-state2}). Then the controller chooses an \emph{unused} id and computes the update commands to be executed (Line~\ref{update:compute}). \liron{I have replaced "claims the first" with "chooses an" following Petr email}
...
The code is publicly available at \url{https://github.com/lironsc/of-sync-lib}. \liron{perhaps we can put it as reference? (putting the link in bib)}
