...
\newcommand{\claimmagic}{\textit{CLAIM}}
\newcommand{\memmagic}{\textit{WRITE}}
...
\small $^1$ Tel Aviv University, Israel \quad $^2$ TU Berlin \& T-Labs,
Germany \quad $^3$ T\'el\'ecom ParisTech, France
}
...
This paper proposes mechanisms to solve such more general 
...
the match patterns $m_1=(c_1\concat x_1, \ones{48})$ and $ m_2=(c_2\concat x_2, \ones{48})$ are distinct,
even if $x_1=x_2$.
...
If the check succeeds, we delete the entry. In any case since the check entry contains the controller id, it doesn't conflicts with other check entries and there is no negative impact of delaying the deletion.
...
The memory \memwrite and \compare
 ...
 As can be seen in Algorithms \ref{alg:write}-\ref{alg:compare},
  the value part of the added flow entry match contains the memory address mask part contains
...
The code is made available together with this paper.
...
different workloads. 
The tests were conducted using a software switch, Open vSwitch (version 2.4.0) and virtual network simulator, Mininet.
...
despite the concurrency, all transactions have been successfully completed.
