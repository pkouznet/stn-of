...
\emph{transactions}.
In particular, we present a powerful interface 
to implement transactions \emph{in-band}, on the 
switch itself
and using
the data plane configuration as a shared memory.
...
This allows us 
to realize fundamental consensus primitives in the presence
of controller failures, and we discuss applications
for 
 consistent policy composition and fault-tolerant control-planes.
Interestingly, 
by leveraging the match-action
paradigm to realize simple key-value stores, 
we can implement our synchronization framework in
today's standard Openflow protocol, 
and we report on
a proof-of-concept accordingly.
\end{abstract}
...
compositions of a user-specic update functions.
...
However, as we will argue, there are often conflicts which are less obvious,
and go beyond simple range overlaps. 
...
\vspace{1mm}
\noindent \textbf{Example: Handling complex dependencies.} We consider two controllers
in charge
...
This paper proposes mechanisms to solve these synchronization problems 
by using a minimal
...
to use a meta-configuration space at
the switches which can be manipulated using our
...
traffic. One way to achieve this, for example, is to set a low priority to
...
\subsection{Background on OpenFlow}
...
However, as we have for example seen in our load-balancing
...
As a proof of concept, we implemented the algorithms as a python library that wraps ovs-ofctl - a well known command line tool for monitoring and administering OpenFlow switches. 
We evaluated the framework using a series of tests that used our low-level abstractions as well as the transactional policy update algorithm.

We used three processes to make 3000 concurrent policy updates, each update consists of replacing a single rule with another one and we verified that all transactions completed correctly. The tests were conducted using a software switch, Open vSwitch (version 2.4.0) and virtual network simulator, Mininet.
This framework can be used as synchronization tool by itself and we also work on to implementing it as as integral part of existing controllers such as Ryu. 


\section{Applications}\label{sec:apps}

We now describe sample scenarios of how of our synchronization mechanisms
can be used for control applications. We aim at two applications: the
simple load-balancing scheme we discussed as an example in the beginning,
and a more sophisticated forwarding policy
...
is given in Algorithm~\ref{alg:naive-lb}.
However, this algorithm fails in the presence of
...
to synchronize their possibly concurrent actions. We believe that our centralized in-band
approach is natural and attractive, as it avoids the
complexities and limitations of distributed out-of-band agreement protocols, which may also
