...
of operations which is performed in an atomic, serializable manner, providing all-or-nothing semantics. 
...
and techniques to solve distributed synchronization and consistency problems. 
...
any non-trivial update conflicts. 
...
Thus, our work also provides the missing link for the read-modify-write object
...
\textbf{Background.} In a nutshell,
Openflow
...
\textbf{Limitations.}  While these features are useful primitives, 
building complex transactions from these features
is a non-trivial task, as we will show in the following. 
A bundle can be understood as a transaction:
 it defines a sequence of read and write operations 
...
This section describes how to introduce more powerful transactions. We first present a new synchronization object to 
...
of an SDN switch, such as reading flow table entries or adding new entries.
...
More formally, our transactional abstraction exports the
...
messages using \textsc{FlowMod} commands.
We therefore assume that each of these OpenFlow messages is marked as  a \emph{bundle} message,
...
\begin{algorithm}[t]
    ...
    \label{alg:commit}
    \begin{algorithmic}[1]
    		\State  \textbf{TODO}: Express sending the
                  \textsf{OFPFF\_BUNDLE\_CONTROL} message with type \textsf{OFPBCT\_COMMIT\_REQUEST}
		\Return $cmd$
    \end{algorithmic}
\end{algorithm}
...
FIXME: load-balancing, consensus, etc.
...
\paragraph{Consensus and replicated state machines}
...
powerful synchronization mechanisms can be implemented
 ...
 This section show how to implement 
...
\section{Implementing $\textit{CAS}$ with FlowMod commands}
