...
\begin{algorithmic}[1]
    ...
    \State $\textit{update\_cmds} \gets \textit{lb\_update}(policy)$ %(new\_flows, ports, policy)$    
    \State $\textbf{send}\{\textit{update\_cmds}\}$
%    \State $\textbf{execute-atomic}\{update\_cmds\}$
    \Statex

    \Statex \emph{\bf Function} $\textit{lb\_update}(\textit{policy})$ % (new\_flows, ports, policy)}$
...
Algorithm~\ref{alg:update} uses our id-claimer abstraction
to bound the number of policy identifiers.
The controller begins by reading the current policy id and claiming it in order to prevent another controller from using it with a
different policy.
Then it reads the configuration again in case the policy was changed just before claiming its id.
Then considering other claims the controller chooses an unused id for the update and computes the the update commands using the UPDATE function.
Finally a transaction is made to verify that the chosen id is not claimed and that the policy wasn't changed by someone else, and to update the policy.
Note that in Algorithm \ref{alg:update}, the meta-configuration is
...
\begin{algorithm}[t]
    ...
    \Ensure installed policy is consistent with previous one
 		...
 		\Repeat
 			...
 			\State $\pid 2,\textit{claims},\textit{policy}\gets$ \textbf{read-config} %switch configuration
 			\If {$\pid\neq \pid 2$}
