...
\newcommand{\checko}{\texttt{chkov}\xspace}
...
We show how our interface can be used for applications such as 
in-band controller consensus, consistent policy installation, or load-balancing.
\end{abstract}
...
In order to avoid conflicts, \emph{FlowMod} can be used together with a the $\checko$ flag ($CHECK\_OVERLAP$):
...
message that could not be applied. The error message must use the \stefan{what is this:} \emph{xid} of the offending message,
...
\begin{enumerate}
\item First, it contains a \emph{create bundle} message: a \textsf{OFPT\_BUNDLE\_CONTROL} message of type
\textsf{OFPBCT\_OPEN\_REQUEST}.
\item The follows a sequence of commands (e.g., \emph{FlowMod}), each wrapped in an 
\emph{add to bundle message} (\textsf{OFPT\_BUNDLE\_\add\_MESSAGE}) with the \emph{bundle identifier}.
\item Finally, a \emph{commit bundle} control message (\textsf{OFPT\_BUNDLE\_CONTROL}) of 
type \textsf{OFPBCT\_COMMIT\_REQUEST}.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Example 1: More complex dependencies.} Let us consider a controller application in charge
of \emph{load-balancing}. 
In a load-balancing application, seemingly independent actions, defined over
completely independent logical flow spaces,
may in fact be dependent due to collocation in the underlying physical network.
To be more concrete, say that the objective of the load-balancer
is to distribute flows equally across links or servers.
If multiple controllers concurrently and independently
update forwarding rules (say, two ``independent'' flows with source IP address ranges $1*$ and $0*$), 
...
which allows controllers to coordinate 
...
\item By performing $\textit{write}(\addr,k)$,
  ...
  example, the offset $\addr$ may be used to identify the policy (identifier),
  but our approach is more general.

\item The operation $\textit{\compare}(\addr,k)$, $\addr,k\in\Nat$,
...
As we will see, the bundle feature is not required to implement this abstraction.
...
With this memory interface we can immediately implement 
...
\begin{itemize}
\item With $\textit{claim}(k)$, $i\in[N]$,
  $k\in\Nat$, a controller $i$ \emph{claims}
  identifier $k$.

\item With $\textit{unclaim}(k)$, $i\in[N]$, $k\in\Nat$, controller $i$
...
flag \textsf{\checko}, and using different actions ($2$ instead of $1$), thereby inflicting 
...
have to be sent as part of a bundle. Therefore, the \textsf{\claimcheck} operation is atomic by nature, 
similar to the \textsf{claim} and \textsf{unclaim} operations, and is implemented as essentially one command.
...
%    \Ensure
    		...
    		%\State $cookie \gets x\concat self$
    		\State $action \gets 2$
    		\State $flag \gets \textsf{OFPFF\_\checko}$
...
but we also set the \textsf{\checko} flag. If the value is not there, 
...
In addition, we present in Algorithm \ref{alg:cas} a simple implementation 
of \emph{compare-and-set (CAS)}, using the two last operations.
...
Using our approach, it is straightforward to implement consensus among the controllers
...
As a second example, let us consider a simple policy composition protocol that
...
policy, then chooses a new unique policy identifier \emph{pid}, checks whether the new
...
struct ofp_flow_mod {
	...
	uint64_t cookie_mask; /* Mask used to restrict the cookie bits
		...
		OFPFC_MODIFY* or OFPFC_\dele*. A value
		of 0 indicates no restriction. */
	/* Flow actions. */
	uint8_t table_id; /* ID of the table to put the flow in.
		For OFPFC_\dele_* commands, OFPTT_ALL
...
%    \Ensure
    		...
    		\State $op \gets \textsf{OFPGC\_\dele}$
    		\State $group\_id \gets old$
    		\State $cmd1\gets GroupMod(op, group\_id) $
    		
    		\State $op \gets \textsf{OFPGC\_\add}$
