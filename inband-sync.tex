...
control modules operating on the data plane should be physically distributed.
...
We argue that a powerful transactional interface should and can be implemented \emph{inband}, using 
...
We discuss how to use our synchronization framework to implement
 consistent policy compositions and replicated control-plane services.
\end{abstract}
...
controllers should be redundant~\cite{onix,stn,onos,elasticon}: a failure 
...
%control-plane
data-plane
events close to their origin~\cite{sharon,devoflow,kandoo,jukka,disco}.
...
are non-trivial tasks.~\cite{cpc,sharon,cap-theorem}
...
network~\cite{network-update,podc15,roger-hotnets,correct,stn}.
...
system~\cite{FLP85,CHT96,cap-theorem}, and the amount of communication
complexity~\cite{Lam98,Lam06}.
...
Interestingly, our mechanisms can be implemented using the \emph{standard OpenFlow} 
protocol, and do not require any extensions as assumed and postulated in recent literature (e.g.,
STN~\cite{stn} or NetPaxos~\cite{netpaxos}). 
Our work may hence also contribute to the ongoing discussion of what can be implemented
in-band in today's OpenFlow protocol~\cite{compute,reclaim},
as well as to possible high-level concurrency objects and 
...
Concretely, in our implementation, a part of the data plane configuration space is
...
implement a generic replicated control-plane service~\cite{Her91,Lam98} in a consistent and
fault-tolerant way, which became a mainstream building block in
modern system design~\cite{bigtable,spanner,zookeeper}.
We then discuss a simple CAS-based concurrent policy-update mechanism.
Finally, we use our synchronization abstraction to provide
the missing link for the read-modify-write object
postulated in~\cite{cpc}. \petr{We do not seem to discuss the latter?}
...
\paragraph{Interaction between the planes: FlowMod commands}
...
\begin{algorithm}[t]
    \caption{$\textit{Naive Load Balancing}$}
    \label{alg:naive-lb}
\begin{algorithmic}[1]

    \Require \emph{new\_flows} as flow entries of the new flows, \emph{ports} as a list of switch output ports.
    \State $policy \gets \textbf{read\_config}$
    \State $update\_cmds \gets Calc\_LB\_update(policy)$ %(new\_flows, ports, policy)$
    \State $\textbf{execute-atomic}\{update\_cmds\}$
    \Statex

    \Statex \emph{\bf Function} $\textit{Calc\_LB\_update(policy)}$ % (new\_flows, ports, policy)}$
	...
	\For {$entry \in new\_flows$}
	    \State $port \gets Argmin_{p\in ports} flows\_per\_port[p]$
...
\caption{\emph{Left:} Without synchronization, the two controllers
may end up unbalancing the flow allocations. \emph{Right:}
...
which refer to executing a sequence of commands $op_1,\ldots,op_k$ in
...
\paragraph{Synchronization primitives}
%
\hide{
...
Con sidering, $m_1$ and $m_2$ from the example above,
with respective actions $a_1=1$ and $a_2=2$,  they overlap
with different actions iff $x_1=x_2$. Therefore, if $m_1$ is used in \claimcheck after $m_2$ used in the claim operation,
a conflict is detected iff $x_1=x_2$, as desired.

Note that for space reasons, when checking for claims, also the deletion of the added tester flow entry is included,
...
We now discuss several applications of our transactional interface.

\textbf{Stefan: Give intro
}%very powerful primitive => any sequential program in repliated way can be imlemented, see also STN paper where we assume we can do this. 

\paragraph{Compare-and-Set}
The pathological scenario described in the example in
...
\emph{succeeds}), otherwise it returns $\true$ (the CAS
...
and, more generally,
to implement any generic replicated state machine~\cite{Her91,Lam98}
...
\begin{algorithm}[t]
    ...
    \Ensure installed policy is consistent with previous one
 		%\Do
 		\Repeat
 			\State $\pid,policy\gets$ \textbf{read-config} %switch configuration
...
grow without bound.
However, as was observed in~\cite{cpc}, the number of currently
...
\begin{algorithm}[t]
    ...
    \Ensure installed policy is consistent with previous one
 		%\Do
 		\Repeat
		 	\State $\pid,claims,policy\gets$ \textbf{read-config} %switch configuration
 			\State $\textbf{execute}\{claim(\pid)\}$
 			\State $\pid 2,claims,policy\gets$ \textbf{read-config} %switch configuration
 			\If {$\pid\neq \pid 2$}
