...
\emph{add to bundle message} (\textsf{OFPT\_BUNDLE\_ADD\_MESSAGE}) with the \emph{bundle identifier}.
...
In order to check if a value is written in a given address, we try to add it the same way as in the \memwrite operation  but we also set the \textsf{\checko} flag. If the value is not there, our action differ from the action of the existing entry thereby inflicting a failure.
