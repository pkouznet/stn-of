...
Given this memory interface, we can immediately implement 
...
the invoking transaction). \liron{CAS requieres to run in a bundle while the other operations disscussed here are not. Maybe we should remove CAS to the consensus subsection as it may confuses here}
...
of \emph{Compare-And-Set (CAS)}, using the two last operations. \liron{meybe move CAS to consensus subsection}
...
fails and takes no effect. \liron{the term composition doesn't make sense now with the new user update function... }
