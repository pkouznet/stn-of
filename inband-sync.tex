...
In a nutshell, \emph{FlowMod} specifies a new flow entry or the modifications to existing flow entries.
The main properties of each flow entry are: 1) A match defined by a ternary patterns (a value and a mask) over packet header fields, 2) Instructions defined by  a list of modifications to packet action set 3) a cookie value used to reference a single or multiple packets.
