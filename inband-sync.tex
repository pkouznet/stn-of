...
directly \emph{in OpenFlow}, i.e., using the \emoh{de facto} standard SDN
protocol. 
...
a ternary pattern over packet header fields
(a value and a mask).
\petr{example?}
...
a modification to an existing flow entry. \petr{The description below
...
A FlowMod command can either add a flow entry or delete 
...
$\FlowMod(\match, \add/\dele, \action, \flags)$, where the first field
...
%In reply the switch will send all the flow entries defined in the switch.
In this paper, we indicate sending this message and gathering the responds (the reply messages containing the flow entries) by \emph{read configuration} statement which also returns the configuration.
...
\begin{enumerate}
\item First, a bundle contains a \emph{create bundle} message: a \texttt{ofpt\_bundle\_control} message of type
...
Bundle execution is considered controller atomic in the sense that all controllers can observe switch states (configurations) that either \emph{before} or \emph{after} the
