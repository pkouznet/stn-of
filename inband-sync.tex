...
completely independent logical flow spaces (say, flows with
...
Here the controller first reads the current policy id and claims it in order to prevent another controller from using it with a
...
Finally, within an atomic transaction (\textbf{execatomic}), it verifies that the chosen policy id is not claimed and that 
the policy has not been concurrently modified, and updates the policy.
...
{\color{red}
We also conjecture that they can be extended to implement atomic modifications of multiple switches.   
}
