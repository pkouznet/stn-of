...
to the associated action (to be immediately applied now or later).\petr{"to be immediately applied now or later" - sounds funny}\liron{I agree it sounds a bit strange, maybe we should explain this later after we disscuss the pipeline}
...
and if they fit, some instructions (the actions)  are executed; \petr{ what is the difference between instruction or action?} \liron{we should explain that: every packet is associated with an action set. This set is empty at the beginning and is changed according to the flow table matches. Each match is asociated with instructions that can add or modify actions in the action set and also to apply (now) all actions in the action set. By default action set is applied at the end of the pipeline.}
...
is the \emph{bundle}. A bundle allows to send multiple commands (e.g., \emph{FlowMod} messages) with the same bundle id and then execute them  by sending a bundle commit message. Theses (bundled) commands are performed with \emph{all-or-nothing semantics}:
...
bundle (i.e., all its commands) took effect. \liron{we need to disscuss controller atomicity and order preservation inside a bundle}
...
\liron{following Petr's comments, I suggest the following paragraph}
At first sight, the Openflow bundle feature seems to provide a simple and generic way for a controller to atomically update switch configuration, similarly to a transaction in shared memory system.
However, while the bundle can indeed be seen
 as a ``mini-transaction'', it comes with several
 limitations, which we discuss in the following.
...
the commands in the bundle essentially do
 not support any conditional \emph{modifications} or computations which depend on any current switch configuration (except to limited types of constraints, e.g., free space).
 }
...
rules were already there), it cannot be easily used
to conditionally modify existing rules .
...
We use the $\checko$ optional flag to make the transaction execution dependent on switch state \liron{ last sentence might a bit contradict what we said in the limitation section}.
...
\petr{
the "resource claimed" part a hard read, the trick of using masks and values is smart, deserves to be explained better
}
...
policy and its id \emph{pid}, executes the local update function ($\textit{apply\_update}()$) which results with update commands and then tries to apply them atomically with updating the $pid$. If, meanwhile, a new
