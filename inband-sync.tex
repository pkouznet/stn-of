...
\section{Use Cases}\label{sec:apps}
...
policy update in a transactional $\execatomic}$ construct together
...
\emh{succeeds}), otherwise it returns $\true$ (the CAS
\emh{fails} (which causes an abort of
...
CAS can be used to implement a wide range powerful abstractions
shared by the controllers, such as queues, sets, hash tables,
etc.~\cite{Her91} in the \emph{wait-free} manner, i.e., tolerating
asynchrony and failure of any number of controllers.      
In our transactional setting, the controllers
