...
We describe an implementation of the fundamental \emph{compare-and-set} primitive, 
...
A single CAS invocation can be used to solve the fundamental
problem of fault-tolerant \emph{consensus}~\cite{FLP85} in which the controllers
...
Using multiple CAS invocations, the controllers can implement any
generic replicated state machine~\cite{Her91,Lam98} on the control plane, 
in the \emph{wait-free} manner, i.e., tolerating
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
    \Require unsafe policy update function $\ufunc$, policy id $\addr$
...
\begin{algorithm}[h]
    ...
    \label{alg:update}
    \begin{algorithmic}[1]
        \Require unsafe policy update function $\ufunc$, a predefined switch (memory) address for policy id: $\paddr$, a predefined id space $C$.
    \Ensure installed policy is consistent with previous one
 		%\Do
 		\Repeat
