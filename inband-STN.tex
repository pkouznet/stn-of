...
\newcommand{\liron}[1]{\textit{\textcolor{green}{[liron]: #1}}} % Comments
...
\emph{unclaim} and \emph{abortIfClaimed} with the following sequential
semantics:
...
\item $\textit{abortIfClaimed}(k)$, $k\in\Nat$, 
...
\item $\textit{abortIfUnequal}(addr,k)$, $addr,k\in\Nat$, 
...
We use creation and deletion of flow entries as abstraction to claiming/unclaiming  resource id numbers and also to checking whether a given id is claimed. The match part of the added flow entry represents the added
