...
\begin{abstract}
The control plane of modern Software-Defined Networks (SDNs)
is \emph{distributed}: a redundant control plane
can ensure a high availability, and a spatially distributed
control plane allows to handle time-critical dataplane
events close to their origins. Moreover, an SDN may 
be managed by multiple administrators simultaneously. 
However, not much is known today about how to implement
such a distributed control plane. 
In this paper, we show that powerful coordination objects
can be implemented using today's Openflow switches.
