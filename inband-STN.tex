...
transactional interface which allows a controller to perform multiple
...
of an SDN switch, such as reading flow tables or adding new rules. 
 In addition, we also assume transactional access to  \emph{multi-set}
abstractions that store \emph{items} and export \emph{add} and \emph{delete} operations with
regular semantics. The multi-set abstractions are used for resolving
conflicts between concurrent updates of the switch configuration and are themselves 
implemented as flow-table components.

A multi-set object also exports operations \emph{cond-add} and \emph{cond-delete} with the following
semantics:
...
\begin{itemize}
\item \textbf{$\textit{cond-add}(x)$:} if $x$ is not in the set, adds 
  $x$ to the set; otherwise, \emph{fails} and, respectively,
  aborts the transaction in which the operation is invoked.

\item \textbf{$\textit{cond-delete}(x)$:} if $x$ is in the set, deletes an
$x$ from the set; otherwise, \emph{fails} and
  aborts the transaction.
\end{itemize}  

Formally, we guarantee that in every execution, all committed
transactions constitute a \emph{legal} sequential
order~\cite{Pap79-serial}. On the progress side, we ensure that a
